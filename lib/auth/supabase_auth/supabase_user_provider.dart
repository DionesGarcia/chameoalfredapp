...
class ChameOAlfredSupabaseUser extends BaseAuthUser {
  ChameOAlfredSupabaseUser(this.user);
  User? user;
  @override
...
Stream<BaseAuthUser> chameOAlfredSupabaseUserStream() {
  ...
  return (!loggedIn
      ...
      .map<BaseAuthUser>(
    (authState) {
      currentUser = ChameOAlfredSupabaseUser(authState?.session?.user);
      return currentUser!;
    },
  );
}
