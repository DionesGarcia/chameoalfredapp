...
class RestauranteSupabaseUser extends BaseAuthUser {
  RestauranteSupabaseUser(this.user);
  User? user;
...
Stream<BaseAuthUser> restauranteSupabaseUserStream() {
  ...
  return (!loggedIn
      ...
      .map<BaseAuthUser>(
    (authState) {
      currentUser = RestauranteSupabaseUser(authState?.session?.user);
      return currentUser!;
    },
  );
}
